...
<?php
    ...
    public function handle_save_managers(): void
    {
        ...
        $allowed  = ['all', 'specific', 'admin_only'];
        $new_mode = isset($_POST['ftb_editor_access_mode']) && in_array($_POST['ftb_editor_access_mode'], $allowed, true)
            ? $_POST['ftb_editor_access_mode']
            : 'all';
        ...
        if ('all' === $new_mode) {
            ...
            foreach (array_map('absint', (array) get_option('ftb_designated_managers', [])) as $id) {
                $user = get_user_by('id', $id);
                if ($user && ! $user->has_cap('manage_options')) {
                    $user->remove_cap('ftb_manage_settings');
                }
            }
            update_option('ftb_designated_managers', []);
            ...
            $old_ids = array_map('absint', (array) get_option('ftb_designated_managers', []));

            ...
            <label class="donation-form__checkbox-label<?php echo ! empty( $errors['gdpr'] ) ? ' donation-form__checkbox-label--error' : ''; ?>">
                ...
                <?php if ( $privacy_url ) :
                    printf(
                        ...
                        wp_kses( __( 'Ik ga akkoord met de %s <span class="donation-form__required" aria-hidden="true">*</span>', 'ftb-donation-form' ), [ 'span' => [ 'class' => [], 'aria-hidden' => [] ] ] ),
                        '<a href="' . esc_url( $privacy_url ) . '" target="_blank" rel="noopener noreferrer">' . esc_html__( 'privacyverklaring', 'ftb-donation-form' ) . '</a>'
                    );
                else : ?>

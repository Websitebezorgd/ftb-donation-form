		...
		<div class="ftb-admin-form__col">
			...
			<section class="ftb-admin-form__section">
				...
				<div class="ftb-admin-form__group">
					...
					$prv_fields = get_option( 'ftb_form_fields', array() );
					$prv_items  = array( __( 'naam', 'ftb-donation-form' ), __( 'e-mailadres', 'ftb-donation-form' ) );
					if ( ! empty( $prv_fields['phone'] ) ) {
						$prv_items[] = __( 'telefoonnummer', 'ftb-donation-form' );
					}
					foreach ( array( 'street', 'house_number', 'postal_code', 'city' ) as $f ) {
		...
		<div class="ftb-admin-form__col">
			...
			<section class="ftb-admin-form__section">
				...
				<div class="ftb-admin-form__group">
					...
					if ( ! empty( $preview_fields['phone'] ) ) {
						$preview_optional[] = __( 'Telefoon:', 'ftb-donation-form' ) . ' 06 12345678';
					}
					if ( ! empty( $preview_fields['street'] ) ) {
						$preview_optional[] = __( 'Straat:', 'ftb-donation-form' ) . ' Dorpstraat';
					}
					if ( ! empty( $preview_fields['house_number'] ) ) {
						$preview_optional[] = __( 'Huisnummer:', 'ftb-donation-form' ) . ' 12';
					}
					if ( ! empty( $preview_fields['postal_code'] ) ) {
						$preview_optional[] = __( 'Postcode:', 'ftb-donation-form' ) . ' 1234 AB';
					}
					if ( ! empty( $preview_fields['city'] ) ) {
						$preview_optional[] = __( 'Plaats:', 'ftb-donation-form' ) . ' Amsterdam';
					}

					$preview_recurring = get_option( 'ftb_enable_recurring', '1' );
					$preview_frequency = '1' === $preview_recurring
						? array( __( 'Frequentie:', 'ftb-donation-form' ) . ' ' . __( 'Maandelijks', 'ftb-donation-form' ) )
						: array();
